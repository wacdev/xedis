...
ava('hset', async(t) => {
  var key, key2, key3, map, val, val2, val3;
  map = KEY + 'hset';
  key = '键';
  val = '值';
  key2 = 'key2';
  ...
  t.deepEqual((await C.hmgetB(map, ['none', 'none'])), [void 0, void 0]);
  t.deepEqual((await C.hmgetB(map, [key3])), [val3]);
  t.deepEqual((await C.hmgetB(map, [key2])), [val2].map(utf8e));
